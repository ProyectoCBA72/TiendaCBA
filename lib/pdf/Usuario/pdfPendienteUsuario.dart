...
class PdfPendienteUsuarioScreen extends StatefulWidget {
  const PdfPendienteUsuarioScreen({
    super.key,
  ...
  @override
  State<PdfPendienteUsuarioScreen> createState() =>
      _PdfPendienteUsuarioScreenState(usuario: usuario, registro: registro);
}

class _PdfPendienteUsuarioScreenState extends State<PdfPendienteUsuarioScreen> {
  final UsuarioModel usuario;

  final List<DataGridRow> registro;

  _PdfPendienteUsuarioScreenState({
    required this.usuario,
    required this.registro,
  });
  ...
  Future<Uint8List> generatePdf(final PdfPageFormat format) async {
    final doc = pw.Document(
      title: 'CBA Mosquera reporte de pedidos pendientes',
    );
    ...
    final tableData = registro.map((item) {
      ...
      // omitiendo la última celda
      return cellsWithIndex.where((entry) {
        // Omitir la última celda
