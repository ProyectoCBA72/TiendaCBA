...
class PdfConstanciaFacturaScreen extends StatefulWidget {
  const PdfConstanciaFacturaScreen({
    super.key,
  ...
  @override
  State<PdfConstanciaFacturaScreen> createState() =>
      _PdfConstanciaFacturaScreenState(usuario: usuario, registro: registro);
}

class _PdfConstanciaFacturaScreenState
    extends State<PdfConstanciaFacturaScreen> {
  final UsuarioModel usuario;

  final List<DataGridRow> registro;

  _PdfConstanciaFacturaScreenState({
    required this.usuario,
    required this.registro,
  });
  ...
  Future<Uint8List> generatePdf(final PdfPageFormat format) async {
    final doc = pw.Document(
      title: 'CBA Mosquera factura de venta',
    );
    ...
    final total = registro.fold(0, (sum, item) {
      final valorPedido = item.getCells()[5].value;
      return sum + (valorPedido is int ? valorPedido : 0);
    });
