...
class PdfRecibidoUnidadScreen extends StatefulWidget {
  const PdfRecibidoUnidadScreen({
    super.key,
  ...
  @override
  State<PdfRecibidoUnidadScreen> createState() =>
      _PdfRecibidoUnidadScreenState(usuario: usuario, registro: registro);
}

class _PdfRecibidoUnidadScreenState extends State<PdfRecibidoUnidadScreen> {
  final UsuarioModel usuario;

  final List<DataGridRow> registro;

  _PdfRecibidoUnidadScreenState({
    required this.usuario,
    required this.registro,
  });
  ...
  Future<Uint8List> generatePdf(final PdfPageFormat format) async {
    final doc = pw.Document(
      title: 'CBA Mosquera reporte de producciones recibidas',
    );
    ...
    final tableData = registro.map((item) {
      ...
      return cellsWithIndex.map((entry) {
        ...
        return index == 6
            ...
            : index == 4 || index == 5 || index == 7 || index == 9
    ...
    doc.addPage(
      pw.MultiPage(
        ...
        build: (final context) => [
          ...
          pw.Center(
            child: pw.Text(
              "Reporte de producciones recibidas",
