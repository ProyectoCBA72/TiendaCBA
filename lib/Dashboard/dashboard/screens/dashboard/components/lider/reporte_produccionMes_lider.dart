...
class ReporteProduccionMesLider extends StatelessWidget {
  ...
  const ReporteProduccionMesLider({super.key, required this.usuario});
  ...
  List<StepLineSeries<ProduccionMesDataLider, String>> _buildSeries(
      List<ProduccionModel> data, UsuarioModel usuario) {
    List<ProduccionModel> unidadesFiltradas =
        data.where((p) => p.unidadProduccion.sede.id == usuario.sede).toList();
    ...
    return unidadesProduccion.map((unidad) {
      return StepLineSeries<ProduccionMesDataLider, String>(
        name:
        ...
        xValueMapper: (ProduccionMesDataLider data, _) =>
  ...
  List<ProduccionMesDataLider> _getProduccionDataMes(String unidadProduccion,
    ...
    // Convierte el mapa de datos en una lista de objetos ProduccionMesDataLider
    return data.entries
        .map((entry) => ProduccionMesDataLider(entry.key, entry.value))
        .toList();
  }
}
